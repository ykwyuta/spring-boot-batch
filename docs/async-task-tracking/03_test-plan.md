...
に記載された分岐表（A0a〜A15、B1〜B6、C0a〜C8、D1〜D3、計36項目）をもとに、C1（分岐網羅: すべての
分岐の真偽・分岐先を最低1回ずつ実行する）の観点でテストケースを設計する。書式は既存の同期API機能の
...
設けない（結合シナリオ「8」で疎通のみ確認する）。同様にA1（JSONとして不正な形式、
`HttpMessageNotReadableException`）も設計書「8.1」「8.3」の方針通り既存ハンドラ
（`handleHttpMessageNotReadableException`）をそのまま再利用するだけで新規ロジックの追加はないが、
AC-01〜AC-07（Bean Validationエラー、`MethodArgumentNotValidException`）とは例外クラス・コード
経路が異なり、これらのケースでは一度も再現されないため、A1自体の分岐（デシリアライズ段階での
失敗が既存ハンドラへ正しくディスパッチされること）を再確認する専用ケース（AC-00）を設ける。

| ケースID | 対象分岐 | 条件式・分岐先 | 入力条件 | 期待結果 |
| :-- | :-- | :-- | :-- | :-- |
| AC-00 | A1（JSONとして不正な形式） | リクエストボディがJSONとして不正な形式の場合、`HttpMessageNotReadableException` を既存の `GlobalExceptionHandler.handleHttpMessageNotReadableException` が捕捉する（既存ハンドラの再利用、新規ロジックの追加なし） | `POST /internal/tasks/execute-async` に `Content-Type: application/json` で本文 `{invalid json` を送信 | HTTPステータス `400 Bad Request`。`errorCode="BAD_REQUEST"`（既存`docs/service-bash-invocation/03_test-plan.md` CT-01/EH-05と同じ値）。`AsyncTaskExecutionService`は呼び出されない |
...
03_test-plan.md` EH-01〜EH-05）の対象であり本書では再掲しない。A1（`HttpMessageNotReadableException`）
についても、本機能は`handleHttpMessageNotReadableException`を一切変更せずそのまま再利用するのみ
であるため（設計書「8.1」「8.3」）、本セクションでは新規の例外ハンドラケースとして扱わない。A1の
分岐自体（デシリアライズ失敗時に当該ハンドラへ正しくディスパッチされること）はセクション1の
AC-00で再現・確認する。新規追加分の3メソッドのみを対象とする。
...
`@SpringBootTest` + `MockMvc`フルコンテキスト（実際の`LocalhostOnlyInterceptor`・実際の
`AsyncTaskExecutorConfig`のスレッドプールを使用）を用いた結合テスト。C1網羅の対象分岐の大半は
セクション1〜7の単体テストで充足済みだが、以下のケースは複数層の結合を経て初めて確認できる
内容（A0a/A0bの新規エンドポイントへの適用確認、同期API・非同期APIの挙動差異）であるため、
必須ケースとして本セクションに置く。

| ケースID | 対象分岐 | 条件式・分岐先 | 入力条件 | 期待結果 |
| :-- | :-- | :-- | :-- | :-- |
| IG-01（必須） | A0a/C0a（新規エンドポイントへの適用） | `LocalhostOnlyInterceptor`が既存設定（`addPathPatterns("/internal/**")`）により新規エンドポイントにも自動適用され、ローカルホストからのアクセスを許可する | テスト実行環境（ローカルホストとみなされる）から`POST /internal/tasks/execute-async`および`GET /internal/tasks/{taskId}`を呼び出す | いずれも403にならず、設計通りのレスポンス（202、200または404）が返る |
| IG-02（必須、AE-08実体） | B3との対比: 同期API・非同期APIの挙動差異（設計書「8.3」、設計レビュー指摘事項） | 同一の業務的失敗（`taskName=task-business-failure`）に対し、同期APIは`422`、非同期APIは完了後`200 OK`＋`status=FAILED`を返す | (a) `POST /internal/tasks/execute`に`{"taskName": "task-business-failure"}`を送信。(b) `POST /internal/tasks/execute-async`に`{"taskName": "task-business-failure", "inputFilePaths": []}`を送信し、返却された`taskId`に対して`status`が`SUCCEEDED`または`FAILED`になるまで`GET /internal/tasks/{taskId}`をポーリングする（ポーリング間隔は短く、タイムアウトを設けたうえで結果を待つ） | (a)はHTTPステータス`422`、`errorCode="TASK_EXECUTION_FAILED"`。(b)は`POST`時点で`202`、ポーリング完了後の最終`GET`応答が`200 OK`・`status="FAILED"`・`message`に前提条件不成立の旨を含む。(b)の一連のHTTPレスポンスの中に`422`は一度も出現しない |
| IG-03（必須） | 正常系のE2E（A12, B1, B5, C7） | 正常系タスク（`sample-task`）を非同期APIで実行し、完了までポーリングして結果を取得できる | `POST /internal/tasks/execute-async`に`{"taskName": "sample-task", "inputFilePaths": []}`を送信し、`taskId`を取得。`status=SUCCEEDED`になるまで`GET /internal/tasks/{taskId}`をポーリングする | `POST`応答が`202`・`status="PENDING"`。最終的な`GET`応答が`200 OK`・`status="SUCCEEDED"`・`outputFilePaths`に1件のパスを含む |
| IG-04（参考・任意） | A10/A11 + C2/C4 のE2E疎通 | 入力ファイル不存在エラー、および未知の`taskId`に対する404応答をE2Eで再確認する | (a) 存在しないファイルパスを指定して`POST /internal/tasks/execute-async`を呼ぶ。(b) ランダムな`UUID`で`GET /internal/tasks/{taskId}`を呼ぶ | (a)(b)いずれも`404`、それぞれ`errorCode="INPUT_FILE_NOT_FOUND"`/`"TASK_NOT_FOUND"` |

注記: IG-01〜IG-03は本テスト計画のC1網羅の必須要件に含める（依頼事項「同期API・非同期APIの
挙動差異を明示的に検証するテストケース」「正常系のE2E疎通」を満たすため）。IG-04は単体テスト
（AC-12、GC-02、GS-02等）で既に分岐自体は検証済みのため参考・任意とする。既存の同期APIの
テスト計画（`docs/service-bash-invocation/03_test-plan.md`セクション5）と同様、
`LocalhostOnlyInterceptor`を経由する結合テストは実行環境の`remoteAddr`に依存するため、CI環境等で
非ローカルホストとして扱われる場合は実装フェーズでテスト環境側の調整（`@LocalServerPort`構成や
モック化）を検討する。
...
| セクション | 対応分岐 | ケース数 |
| :-- | :-- | :-- |
| 1. `AsyncTaskExecutionController`（実行受付API） | A1〜A6, A10〜A13, A14, A15 | 13（AC-00〜AC-12） |
| 2. `AsyncTaskExecutionService.acceptAsyncExecution` | A7〜A13 | 7（AS-01〜AS-07） |
| 3. `AsyncTaskExecutionService.executeAsync`（非同期実行本体） | B1〜B6 | 8（AE-01〜AE-08、AE-08は結合シナリオIG-02で実体化） |
| 4. `AsyncTaskExecutionStateStore` | （基盤、分岐番号なし） | 4（SS-01〜SS-04） |
| 5.1 `AsyncTaskExecutionController.getStatus` | C1, C2 | 2（GC-01〜GC-02） |
| 5.2 `AsyncTaskExecutionService.getStatus` ＋ レスポンス組み立て | C3〜C8 | 6（GS-01〜GS-06） |
| 6. `GlobalExceptionHandler`（新規追加分） | A10/A11/A13・C2/C4 のハンドラ側 | 4（EH-A01〜EH-A04） |
| 7. `AsyncTaskRetentionScheduler` | D1〜D3（+境界値1件） | 4（RT-01〜RT-04） |
| 8. 結合シナリオ | A0a/C0a, B3（差異）, A12/B1/B5/C7, A10/A11/C2/C4 | 4（IG-01〜IG-04、うちIG-04は参考） |
| 合計 | - | 52（結合シナリオの参考ケース1件を含む。必須ケースのみでは51） |

設計書「9」の分岐表に挙げられた全分岐について、真偽（または各分岐先）を最低1回ずつ実行する
ケースが上記表に含まれていることを以下の対応関係で確認済み。

| 分岐番号 | 対応ケースID |
| :-- | :-- |
| A0a | IG-01 |
| A0b | （既存`LocalhostOnlyInterceptorTest`のIT-03で網羅済み、本機能でのIT追加は不要） |
| A1 | AC-00 |
| A2 | AC-01, AC-02 |
| A3 | AC-03 |
| A4 | AC-04, AC-05 |
...
| A8 | AS-02 |
| A9 | AS-03 |
| A10 | AS-04（Service側本体）, AC-12（Controller側伝播） |
| A11 | AS-05 |
| A12 | AS-06, AC-08 |
| A13 | AS-07（Service側本体）, AC-11（Controller側伝播） |
| A14 | AC-09 |
| A15 | AC-10 |
| B1 | AE-02, AE-06 |
| B2 | AE-03 |
...
| B6 | AE-07 |
| C0a | IG-01 |
| C0b | （既存`LocalhostOnlyInterceptorTest`のIT-03で網羅済み） |
| C1 | GC-01 |
| C2 | GC-02 |
| C3 | GS-01 |
| C4 | GS-02 |
...
以上より、設計書「9」に列挙された全分岐（A0a〜A15、B1〜B6、C0a〜C8、D1〜D3、計36項目）に
対し、最低1ケース以上が対応付けられており、C1（分岐網羅）を満たす。
