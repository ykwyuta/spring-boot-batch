...
| セクション | 対応分岐 | ケース数 |
| :-- | :-- | :-- |
| 1. `AsyncTaskExecutionController`（実行受付API） | A1〜A6, A10〜A13, A14, A15 | 12（AC-01〜AC-12） |
...
| 合計 | - | 51（結合シナリオの参考ケース1件を含む。必須ケースのみでは50） |
...
| A1 | （既存`HttpMessageNotReadableException`捕捉は既存ハンドラ再利用のため、AC-01〜AC-07のバリデーション系ケースで`@Valid`経路を確認すれば十分。専用ケースを設けるかは実装フェーズでAC-01に統合してよい） |
| A2 | AC-01, AC-02 |
| A3 | AC-03 |
| A4 | AC-04, AC-05 |
| A5 | AC-06 |
...
以上より、設計書「9」に列挙された全分岐（A0a〜A15、B1〜B6、C0a〜C8、D1〜D3、計34項目）に
